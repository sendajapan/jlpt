...
class VocabularyService
{
    public function getAll(array $filters = [], int $perPage = 100): LengthAwarePaginator
    {
        return Vocabulary::query()
            ...
            ->when(isset($filters['audio_flag']) && $filters['audio_flag'] === 'pending', fn($q) => $q->whereAny(['sentence_audio_en_reviewed','sentence_audio_en_reviewed','sentence_audio_jp_reviewed','audio_en_reviewed'], '0')
                ->whereNotNull(['sentence_audio_en','sentence_audio_en','sentence_audio_jp','audio_en'])
            )
            ->when(isset($filters['audio_flag']) && $filters['audio_flag'] === 'complete', fn($q) => $q->where('sentence_audio_en_reviewed', '1')
