...
class VocabularyService
{
    public function getAll(array $filters = [], int $perPage = 100): LengthAwarePaginator
    {
        return Vocabulary::query()
            ...
            ->when(isset($filters['audio_flag']) && $filters['audio_flag'] === 'pending', fn($q) => $q->where('sentence_audio_en_reviewed', '0')
                ->orWhere('audio_jp_reviewed', '0')
                ->orWhere('sentence_audio_jp_reviewed', '0')
                ->orWhere('audio_en_reviewed', '0')
            )
