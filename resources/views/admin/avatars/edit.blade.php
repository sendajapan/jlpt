...
<div class="bg-white rounded-lg border border-zinc-200 overflow-hidden">
    <div class="flex items-center justify-between px-5 py-3 border-b border-zinc-100">
        <div>
            <p class="text-sm font-semibold text-zinc-900">Edit Avatar</p>
            <p class="text-xs text-zinc-500 mt-0.5">Leave image field empty to keep the current image</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.avatars.update', $avatar) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input
                        type="number"
                        name="coin_price"
                        value="{{ old('coin_price', $avatar->coin_price) }}"
                        min="0"
                        placeholder="0 = free"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-medium text-zinc-700 mb-1.5">Avatar Image</label>
                    @php $currentUrl = Storage::url($avatar->image_path); @endphp
                    <div x-data="{ preview: @js($currentUrl) }">
                        <div class="w-32 h-32 rounded-xl border-2 border-zinc-200 bg-zinc-50 overflow-hidden flex items-center justify-center mb-2">
                        ...
                        <input
                            ...
                            name="image_path"
                            accept="image/*"
                            x-on:change="preview = $event.target.files[0] ? URL.createObjectURL($event.target.files[0]) : @js($currentUrl)"
                        ...
                        >
                        <p class="mt-1 text-[10px] text-zinc-400">Leave empty to keep current. New image auto-resized to 512×512.</p>
                        @error('image_path')
                            <p class="mt-1 text-[10px] text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input
                            type="number"
                            name="sort_order"
                            value="{{ old('sort_order', $avatar->sort_order) }}"
                            min="0"
                    ...
                    <div>
                        <label class="flex items-center gap-2 cursor-pointer select-none">
                            ...
                            <input
                                ...
                                name="is_active"
                                value="1"
                                {{ old('is_active', $avatar->is_active) ? 'checked' : '' }}
