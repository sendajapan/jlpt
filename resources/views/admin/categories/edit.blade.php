...
<div class="bg-white rounded-lg border border-zinc-200 overflow-hidden">
    <div class="flex items-center justify-between px-5 py-3 border-b border-zinc-100">
        <div>
            <p class="text-sm font-semibold text-zinc-900">Edit Category</p>
            <p class="text-xs text-zinc-500 mt-0.5">{{ $category->name_en }}</p>
        </div>
    </div>
        ...
        <div class="p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input
                        type="text"
                        name="name_jp"
                        value="{{ old('name_jp', $category->name_jp) }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        type="text"
                        name="name_romaji"
                        value="{{ old('name_romaji', $category->name_romaji) }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    @if($category->icon_path)
                        <div class="mb-2 flex items-center gap-2">
                            <img src="{{ Storage::url($category->icon_path) }}" alt="{{ $category->name_en }}" class="w-8 h-8 rounded object-cover border border-zinc-200">
                            <span class="text-[10px] text-zinc-400 truncate max-w-[140px]">{{ basename($category->icon_path) }}</span>
                        </div>
                    @endif
                    <input
                        type="file"
                    ...
                    >
                    <p class="mt-1 text-[10px] text-zinc-400">{{ $category->icon_path ? 'Leave empty to keep current.' : 'JPG, PNG, SVG, WebP' }}</p>
                    @error('icon_path')
                        <p class="mt-1 text-[10px] text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if($category->audio_path)
                        <audio controls class="h-7 w-full mt-1 mb-2">
                            <source src="{{ Storage::url($category->audio_path) }}">
                        </audio>
                    @endif
                    <input
                        type="file"
                        ...
                        class="block w-full text-xs text-zinc-500 file:mr-3 file:h-7 file:px-3 file:rounded file:border-0 file:text-xs file:font-medium file:bg-zinc-100 file:text-zinc-700 hover:file:bg-zinc-200 transition-colors cursor-pointer"
                    >
                    <p class="mt-1 text-[10px] text-zinc-400">{{ $category->audio_path ? 'Leave empty to keep current.' : 'MP3, WAV, OGG, AAC' }}</p>
                    @error('audio_path')
                        <p class="mt-1 text-[10px] text-red-500">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                <label class="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    <input
                        ...
                        id="is_premium"
                        value="1"
                        {{ old('is_premium', $category->is_premium) ? 'checked' : '' }}
