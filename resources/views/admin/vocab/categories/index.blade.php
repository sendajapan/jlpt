                                ...
                                @method('PATCH')
                                    @if($category->icon_path)
                                        <div x-data="{ hovered: false }" class="relative inline-block" @mouseenter="hovered = true" @mouseleave="hovered = false" onclick="event.stopPropagation()">
                                            <img src="{{ \Illuminate\Support\Facades\Storage::url($category->icon_path) }}" alt="" class="mx-auto rounded object-cover border border-zinc-200 transition-opacity" :class="hovered ? 'opacity-50' : 'opacity-100'" style="border-radius:20px; max-height:150px; max-width:150px; background-size:contain; background-repeat:round; background-image:url('{{ asset($category->vocab_bg_path) }}')">
                                            <div x-show="hovered" class="absolute inset-0 flex items-center justify-center gap-1.5" style="display:none">
                                                <a href="{{ \Illuminate\Support\Facades\Storage::url($category->icon_path) }}" target="_blank" onclick="event.stopPropagation()" class="inline-flex items-center h-6 px-2 rounded bg-white/90 border border-zinc-200 text-[10px] font-medium text-zinc-700 hover:bg-white transition-colors shadow-sm">Preview</a>
