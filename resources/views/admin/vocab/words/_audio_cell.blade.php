...
<div x-data="{
    ...
    <template x-if="!gone && !url && {{ $existing ? 'true' : 'false' }}">
        <div id="id_{{$field}}" class="inline-flex items-center gap-1">
            ...
            <button @click="window.open('https://pathlingo.scholarlyapps.com/audio_trim.php?audio={{ $audio_name }}', '_blank')" class="inline-flex h-7 items-center gap-1 px-2 rounded border border-green-200 bg-white hover:bg-blue-50 text-[10px] font-medium text-green-600 transition-colors" >
                Cut
            </button>
