                            ...
                            <td class="w-30 px-2 py-3 text-center border-r border-zinc-100">
                                <form method="POST" action="{{ route('admin.vocab.words.update-image', $vocab) }}" enctype="multipart/form-data">
                                    ...
                                    <label class="cursor-pointer block group">
                                        @if($vocab->image_path)
                                            <img src="{{ \Illuminate\Support\Facades\Storage::url($vocab->image_path) }}" alt="" class="w-30 h-30 mx-auto rounded object-cover border border-zinc-200 group-hover:opacity-60 transition-opacity" style="max-height: 150px;max-width: 150px;">
                                        @else
